  ...
  for(j in 1:length(nN)){
    ...
    for(k in 1:N){
      ...
      if(class(ans_em) == "mixEM"){
        ...
        estIm_em = NULL #t(matrix(rep(colMeans(y), nrow(y)), ncol(y)))
      }
      ...
      for(i in 1:length(perm)){
        ...
        accA_perm[i] <- max(rowSums((estA[,perm[[i]]] - trA)^2))/n
        
        mse_nmf_perm[i] <- max(rowSums((estOm_nmf[perm[[i]],] - trOm)^2))/M
        accA_nmf_perm[i] <- max(rowSums((estA_nmf[,perm[[i]]] - trA)^2))/n
