...
title(main = paste0("m = ",m, ", A = 0,...,", max(al), ", sigma = ",sigma), outer = T) #", number of MC runs = ", mc_runs
...
plot(nN[ind], accANm[ind,1], type = "l", ylim = c(0,1), xlab = "n",ylab = expression(paste('mean( ', F ==  hat(F), ' )')), col = colM[1], lwd = 2) 
for(i in 2:ncol(mseNm)){
